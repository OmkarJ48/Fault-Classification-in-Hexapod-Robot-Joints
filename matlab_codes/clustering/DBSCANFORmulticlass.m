...
labels2 = multiclassData{:, end};  % Extract labels (last column)
data3 = multiclassData{:, 1:end-1};  % Extract features (all but last column)

% Create new balanced dataset
balancedData3 = array2table(data3, 'VariableNames', binaryData.Properties.VariableNames(1:end-1));
balancedData3.Label = labels2;  % Add back label column

% Normalize features using Z-score
features = balancedData3{:, 1:end-1};  % Extract feature values
balancedmul = zscore(features);  % Standardize data

% Feature Selection using Relief Algorithm
fprintf('Performing Feature Selection...\n');
[ranking, weights] = relieff(balancedmul, balancedData3.Label, 10);
num_features_to_select = ceil(size(balancedmul, 2) * 0.7);  % Select top 70% features
selected_features = balancedmul(:, ranking(1:num_features_to_select));
...
original_labels = double(multiclassData{:, end});
