% Load data 
labels = binaryDataSubset2{:, end};  % Extract labels (last column)
data = binaryDataSubset2{:, 1:end-1};  % Extract features (all but last column)

% Apply SMOTE for Class Balancing
minority_class = 0;  % Adjust based on dataset
N = 150;  % Oversampling percentage
k = 5;  % Number of nearest neighbors
[data_smote, labels_smote] = smote(data, labels, minority_class, N, k);

% Create new dataset with SMOTE
balancedData = array2table(data_smote, 'VariableNames', binaryDataSubset2.Properties.VariableNames(1:end-1));
balancedData.Label = labels_smote;  % Add back label column

% Normalize features using Z-score
features = balancedData{:, 1:end-1};  % Extract feature values
balanced_features = zscore(features);  % Standardize data

% Feature Selection using Relief Algorithm
fprintf('Performing Feature Selection...\n');
[ranking, weights] = relieff(balanced_features, balancedData.Label, 10);
num_features_to_select = ceil(size(balanced_features, 2) * 0.7);  % Select top 70% features
selected_features = balanced_features(:, ranking(1:num_features_to_select));
